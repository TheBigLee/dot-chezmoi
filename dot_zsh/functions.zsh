...
sshop_update () {
    cp ~/.ssh/sshop_config ~/.ssh/sshop_config.previous &&
    ssh management2.corp.vshn.net -- "sshop --output-archive /dev/stdout" | tar -C ~ -xvzf - &&
    diff -u ~/.ssh/sshop_config.previous ~/.ssh/sshop_config
}
